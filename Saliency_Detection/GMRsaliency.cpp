...
void GMRsaliency::writeMatToFile(cv::Mat& mm, const char* filename)
{
    cv::Mat m;
    mm.convertTo(m,CV_64FC1);
...
Mat GMRsaliency::GetSal(Mat &img)
{
    ...
    Mat bdyt,bdyd,bdyl,bdyr;

	bdyt=GetBdQuery(suplabel,1);
	salt=optAff*bdyt;
	normalize(salt, salt, 0, 1, NORM_MINMAX);
	salt=1-salt;

	bdyd=GetBdQuery(suplabel,2);
	...
	bdyl=GetBdQuery(suplabel,3);
	sall=optAff*bdyl;
	normalize(sall, sall, 0, 1, NORM_MINMAX);
	sall=1-sall;

	bdyr=GetBdQuery(suplabel,4);
	//writeMatToFile(bdyl,"1.txt");
	salr=optAff*bdyr;
...
// ======= do test ========
    Mat salbb;
    salbb = bdyt + bdyd+ bdyl +bdyr;
    threshold(salbb,salbb,0.5,1,THRESH_BINARY);
    Mat newbb = optAff * salbb;
    normalize(newbb, newbb, 0, 1, NORM_MINMAX);
	newbb = 1-newbb;

	cout << salbb.size() << endl;
	salbb.convertTo(salbb,CV_32FC1);
	cout << salbb.type() << endl;
    //writeMatToFile(salbb,"test.txt");


// =========================
// stage 2
	double thr=mean(newbb)[0];
	Mat fgy;
	threshold(newbb,fgy,thr,1,THRESH_BINARY);
