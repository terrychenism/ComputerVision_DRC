...
int main( int argc, char** argv ){
   ...
   Mat img2 =imread( "./image/r_undistorted_perspective.jpg", 1 );
    ...
    //double alpha = 0.5;
int alpha_= 275., beta_= 90., gamma_=220.;
    int f_ = 1390, dist_ =530,dx_ = 90, dy_= 0;
...
while(true) {
   ...
   gamma = ((double)gamma_ - 90.)*CV_PI/180;
    ...
    Mat TR = (Mat_<double>(4,4) <<
    	a*a+b*b-c*c-d*d, 2*b*c-2*a*d, 2*b*d+2*a*c, dx,
    	2*b*c+2*a*d, a*a-b*b+c*c-d*d, 2*c*d-2*a*b,dx,
    	2*b*d-2*a*c,2*c*d+2*a*b, a*a-b*b-c*c+d*d,dist,
    	0,0,0,1);
	...
	double theta = atan(sqrt(pow(x,2.0) + pow(y,2.0)/z));
	double phi = atan(y/x);


	Mat SR =(Mat_<double>(4,4) <<
		sin(theta)*cos(phi), cos(theta)*cos(phi), -sin(phi),0,
		sin(theta)*sin(phi), cos(theta)*cos(phi),cos(phi), 0,
		cos(theta), -sin(theta), 0,0,
		0,0,0,1);

	//Mat transfo = A2 *(T*A1);

    Mat transfo = A2 * (R*(T * (SR * A1)));
