...
static void onTrackbar1(int, void*)
{
  ...
  cv::Mat mapx_persp = Mat(mapx_persp1); // to copy the data
  cv::Mat mapy_persp  = Mat(mapy_persp1); // to copy the data
 
 // for( int j = 0; j < src1.rows; j++ ){ 
	//for( int i = 0; i < src1.cols; i++ ){
	//	if( i > src1.cols*0.25 && i < src1.cols*0.75 && j > src1.rows*0.25 && j < src1.rows*0.75 )
	//		{
	//		// mapx_persp.at<float>(j,i) = 2*( i - src1.cols*0.25 ) + 0.5 ;
	//		//mapy_persp.at<float>(j,i) = 2*( j - src1.rows*0.25 ) + 0.5 ;
	//			   
	//		}
	//	else
 ...
 // }
  ...
  remap(src1, dst_persp1, mapx_persp, mapy_persp, CV_INTER_LINEAR, BORDER_CONSTANT, Scalar(0,0,0) );
...
static void onTrackbar2(int, void*)
{
  ...
  cv::Mat mapx_persp = Mat(mapx_persp2); // to copy the data
  cv::Mat mapy_persp  = Mat(mapy_persp2); // to copy the data
 


  remap(src2, dst_persp2, mapx_persp, mapy_persp, CV_INTER_LINEAR, BORDER_CONSTANT, Scalar(0,0,0) );
...
{   
  ...
  //src1 = imread("./test_fisheye2.jpg" );
  //CvSize size = src1.size();
  //dst_persp   = cvCreateImage( cvSize(1500,1500) , 8, 3 );   
  ...
  //mapx_persp1 =cvCreateMat(1500, 1500, CV_32FC1);
  //mapy_persp1 = cvCreateMat(1500, 1500, CV_32FC1);
  //namedWindow("rectified left image ", 1);
  // namedWindow("rectified right image ", 1);
  //createTrackbar("scaling factor", "rectified image", &sf, 10, onTrackbar);
  onTrackbar1(0, 0);
  onTrackbar2(0, 0);
...
// remap(src1, dst_persp, mapx_persp, mapy_persp, CV_INTER_LINEAR, BORDER_CONSTANT, Scalar(0,0, 0) );
  ...
  //cvNamedWindow( "Original fisheye left camera image", 1 );
  imshow( "Original fisheye  left camera image", src1 );
  
 // cvNamedWindow( "Original fisheye right camera image", 1 );
  imshow( "Original fisheye  right camera image", src2 );
   imshow( "rectified image1", dst_persp1 );
   imshow( "rectified image2", dst_persp2 );
 ...
 // imshow( "Undistorted Perspective Image", dst_persp );
  ...
  // 
  //imshow( "rectified image2", dst_persp2 );
 //imwrite("undistorted_perspective2.jpg",dst_persp1);
